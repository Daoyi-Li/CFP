...
class ACFQLAgent_Finetune(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        def critic_loss_only(params):
            q_for_grad = self.network.select("critic")(
            batch["observations"],
            actions=batch_actions,
            params=params,
            )

            loss = (jnp.square(q_for_grad - target_q) * batch['valid'][..., -1]).mean()

            return loss

        all_critic_loss_grads = jax.grad(critic_loss_only)(grad_params)

        critic_grads = all_critic_loss_grads["modules_critic"]
        ...
        return critic_loss, {
            ...
            'td_error_abs': jnp.abs(q - target_q).mean(),
            'q_max': q.max(),
            'q_min': q.min(),
    ...
    def warmup_actor_loss(self, batch, grad_params, rng):
        """Compute the FQL actor loss."""
        if self.config["action_chunking"]:
            batch_actions = jnp.reshape(batch["actions"], (batch["actions"].shape[0], -1))  # fold in horizon_length together with action_dim
        else:
        ...
        if self.config["actor_type"] == "distill-ddpg":
            ...
            q = jnp.mean(qs, axis=0)
            q_loss = -q.mean()
            def q_loss_only(params):
                act = self.network.select('actor_onestep_flow')(batch['observations'], noises, params=params)
                act = jnp.clip(act, -1, 1)
                qs_inner = self.network.select('critic')(batch['observations'], actions=act)
                return -jnp.mean(qs_inner, axis=0).mean()

            detached_params = jax.tree_util.tree_map(jax.lax.stop_gradient, grad_params)

            q_loss_grad = jax.grad(q_loss_only)(detached_params)

            q_loss_grad_norm = jnp.sqrt(sum(
                jnp.sum(jnp.square(x)) for x in jax.tree_util.tree_leaves(q_loss_grad) if x is not None
            ))
        else:
            q_loss_grad = jax.tree_util.tree_map(lambda x: jnp.zeros_like(x), grad_params)
            q_loss_grad_norm = jnp.zeros(())
            distill_loss = jnp.zeros(())         
            q_loss = jnp.zeros(())
        ...
        return actor_loss, {
            ...
            'bc_flow_loss': bc_flow_loss,
            'distill_loss': distill_loss,
            'q_loss_grad_norm': q_loss_grad_norm,     
        }
    ...
    @jax.jit
    def warmup_total_loss(self, batch, grad_params, rng=None):
        info = {}
        rng = rng if rng is not None else self.rng
        ...
        # Use online actor loss
        actor_loss, actor_info = self.warmup_actor_loss(batch, grad_params, actor_rng)
        for k, v in actor_info.items():
            info[f'actor/{k}'] = v
    ...
    @staticmethod
    def _warmup_update(agent, batch):
        """Online update the agent and return a new agent with information dictionary."""
        new_rng, rng = jax.random.split(agent.rng)

        def loss_fn(grad_params):
            return agent.warmup_total_loss(batch, grad_params, rng=rng)
    ...
    @jax.jit
    def offline_batch_update(self, batch):
        """Update the agent and return a new agent with information dictionary."""
    ...
    @jax.jit
    def warmup_batch_update(self, batch):
